...
| `codex-api/src/endpoint/compact.rs` | compact 路径是 `POST /responses/compact`，响应形态为 `{ output: ResponseItem[] }` | `backend/proxy_fastapi.py` 的 `/v1/responses/compact` 与当前 Codex 协议一致 |
...
└───────────────────────────┬────────────────────────────────┘
                            ▼
┌────────────────────────────────────────────────────────────┐
│ backend/web_server.py + React/Vite                          │
│ - 上下文地图 / 工作台 API                                   │
...
| 模块 | 默认端口 | 职责 |
|---|---:|---|
| `backend/proxy_fastapi.py` | 8787 | Codex-compatible Responses 代理 |
| `backend/web_server.py` | 8765 | HashCode 工作台后端和静态资源服务 |
...
`backend/proxy_fastapi.py` 中的 `ProxySession` 是代理侧核心状态：
...
`backend/web_server.py` 中的 `SessionState` 是工作台后端的会话状态：
...
注意：当前项目的 `backend/web_server.py` 是工作台后端入口，具体能力已经拆到 `backend/web_context.py`、`backend/web_runtime.py`、`backend/web_state.py` 和 `backend/web_handler.py`。
...
当前前端小窗的主路径是 proxy session：`WorkbenchWindow.tsx` 读取 `/api/proxy/sessions`，找到 active session 后调用 `/api/proxy-sync-session` 把代理 transcript 同步进 `backend/web_server.py`。
...
4. 工作台修改 transcript 后，调用 `/api/proxy-session-override` 写回 `backend/proxy_fastapi.py`。
...
```powershell
python -m py_compile backend/proxy_fastapi.py backend/web_server.py scripts\test_compact_proxy.py
npm run typecheck
npm run build
npm run test:compact-proxy
```
