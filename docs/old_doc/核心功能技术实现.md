...
| `codex-api/src/endpoint/compact.rs` | compact 路径是 `POST /responses/compact`，响应形态为 `{ output: ResponseItem[] }` | `backend/proxy_server.py` 的 `/v1/responses/compact` 与当前 Codex 协议一致 |
...
| 模块 | 默认端口 | 职责 |
|---|---:|---|
| `backend/proxy_server.py` | 8787 | Codex-compatible Responses 代理 |
...
`backend/proxy_server.py` 中的 `ProxySession` 是代理侧核心状态：
...
4. 工作台修改 transcript 后，调用 `/api/proxy-session-override` 写回 `backend/proxy_server.py`。
...
```powershell
python -m py_compile backend/proxy_server.py backend/web_server.py scripts\test_compact_proxy.py
npm run typecheck
npm run build
npm run test:compact-proxy
```
